...
class Toy(BaseWeb):
    ...
    def get_articles(self, begin, token, fakeid):
        ...
        Args:
            ...
            fakeid (str): 公众号ID
        """
        url = f"https://mp.weixin.qq.com/cgi-bin/appmsg?token={token}&lang=zh_CN&f=json&ajax=1&random={random.uniform(0, 1)}&action=list_ex&begin={begin}&count=10&query=&fakeid={fakeid}&type=9"
        response = requests.get(url, headers=self.headers, cookies=self.cookies, timeout=(30, 60))
    ...
    def play(self):
        ...
        for 公众号昵称 in 公众号昵称列表:
            ...
            for i in range(0, 1000, 10):
                ...
                app_msg_list = resp_json.get("app_msg_list", [])
